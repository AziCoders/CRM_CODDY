...
def parse_student_data(text: str) -> Dict[str, Any]:
    ...
    Формат: Ключ: значение (каждая строка отдельно)
    """
    data = {}
    text = text.strip()
                ...
                if key == "Возраст":
                    if value:
                        try:
                            ...
                            data[key] = value  # Оставим строку для валидации
                elif key == "Дата поступления":
                    if value:  # Добавляем только если значение не пустое
                        data[key] = value
                elif key in ["ФИО", "Номер родителя"]:
                    # Обязательные поля - добавляем даже если пустые (валидация поймает)
                    data[key] = value
                elif key in ["Имя родителя", "Ссылка на WA, TG", "Комментарий"]:
                    # Необязательные поля - добавляем только если не пустые
                    if value:
                        data[key] = value
                elif key in ["Тариф", "Статус"]:
                    # Эти поля могут быть пустыми - используем значения по умолчанию
                    if value:
                        data[key] = value
                else:
                    # Неизвестные поля - добавляем только если не пустые
                    if value:
                        data[key] = value
...
def prepare_student_data(data: Dict[str, Any], city_name: str) -> Dict[str, Any]:
    ...
    result = {
        "ФИО": data.get("ФИО", ""),
        "Возраст": int(data.get("Возраст", 0)) if data.get("Возраст") else 0,
        "Дата поступления": data.get("Дата поступления", ""),
        "Номер родителя": data.get("Номер родителя", ""),
        "Имя родителя": data.get("Имя родителя", "") or "",  # Пустая строка если не указано
        "Тариф": data.get("Тариф", "Группа 2 раза"),
        "Статус": data.get("Статус", "Обучается"),
        "Город": city_name,
        "Ссылка на WA, TG": data.get("Ссылка на WA, TG", "") or "",  # Пустая строка если не указано
        "Комментарий": data.get("Комментарий", "") or "",  # Пустая строка если не указано
    }
...
async def process_group_selection(
    ...
    # Получаем название группы и информацию о ней
    groups = group_service.get_city_groups(city_name)
    ...
    if city_seats > 0:
        ...
        if free_places <= 0:
            try:
                await callback.message.edit_text(
                    f"❌ В группе '{group_name}' нет свободных мест.\n\n"
                    f"🏙️ Город: {city_name}\n"
                    f"🏫 Группа: {group_name}\n"
                    f"👥 Занято: {total_students}/{city_seats}",
                    reply_markup=get_groups_keyboard(groups)
                )
            except Exception:
                # Если не удалось отредактировать (сообщение уже такое же), просто показываем алерт
                pass
            await callback.answer("❌ В группе нет свободных мест", show_alert=True)
            return

    await state.update_data(selected_group_id=group_id, selected_group_name=group_name)

    # Первое сообщение - информация о городе, группе и обязательных полях
    first_message = (
        f"🏙️ Город: {city_name}\n"
        f"🏫 Группа: {group_name}\n\n"
...
async def send_student_notifications(
    ...
    # Формируем текст уведомления (одно сообщение, как было изначально)
    ...
    try:
        for user in managers_and_owner:
            ...
            try:
                ...
                print(f"❌ Ошибка отправки уведомлений пользователю {user_id} ({user.get('fio', 'N/A')}): {e}")
                import traceback
                traceback.print_exc()
                continue
